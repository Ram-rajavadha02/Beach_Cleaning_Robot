...
class GoalToGoal:
    ...
    def orient_towards_goal(self):
        ...
        while not rospy.is_shutdown():
            ...
            if (-3.15 < self.current_yaw < -1.57 or 1.57 < self.current_yaw < 3.15) and (-3.15 < target_angle < -1.57 or 1.57 < target_angle < 3.15) and (target_angle*self.current_yaw < 0):
                if target_angle > 0 and self.current_yaw < 0:
                    if abs(target_angle) > abs(self.current_yaw):
                        angle_diff = -(abs(target_angle) - abs(self.current_yaw))
                    else:
                        angle_diff = abs(target_angle) - abs(self.current_yaw)
                if target_angle < 0 and self.current_yaw > 0:
                    if abs(target_angle) > abs(self.current_yaw):
    ...
    def move_to_goal(self, target_x, target_y):
        ...
        while not rospy.is_shutdown():
            if not self.obstacle_detected:
                # Check and adjust orientation towards the goal if needed
                target_angle = math.atan2(self.target_y - self.current_y, self.target_x - self.current_x)
                angle_diff = target_angle - self.current_yaw

                if abs(angle_diff) > 0.01:
                    # If the angle difference is greater than 0.01, orient towards the goal
    ...
    def obstacle_avoidance(self):
        ...
        if left_obstacle and right_obstacle:
            ...
            twist_msg = Twist()
            twist_msg.linear.x = 0.1  # Stop linear motion
